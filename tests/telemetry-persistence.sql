...
BEGIN
  reading := jsonb_build_array(jsonb_build_object('tag_id','OHT4-LT','section','oht',
  ...
  PERFORM public.ingest_telemetry(reading);
  SELECT * INTO observed FROM public.telemetry_latest WHERE tag_id='OHT4-LT';
  ...
  SELECT count(*),max(value) INTO history_count,history_value FROM public.historian_logs
    WHERE tag_id='OHT4-LT' AND timestamp=to_timestamp(floor(extract(epoch FROM at_time)/300)*300);
